...
const searchPath = path.join(dataDir, `${prefix}_search.json`);
const catalogPath = path.join(dataDir, `${campusKey}_buildings_catalog.json`);
const floorFiles = (campusConfig.floors || ["0"]).map((floor) => ({
  floor: Number(floor),
  path: path.join(dataDir, `${prefix}_${floor}.json`),
}));
...
const syncCatalog = (searchBuildings, existingCatalog) => {
  ...
  for (const feature of searchBuildings) {
    ...
    byId.set(id, {
      ...
      shortName: current.shortName || id,
...
function main() {
  ...
  console.log(`Edificios sincronizados desde el índice de búsqueda: ${searchBuildings.length}`);
