...
static CollectorOptions LoadOptions(string path)
{
    ...
    options.SourceName = string.IsNullOrWhiteSpace(options.SourceName) ? "Pireon Network Collector" : options.SourceName.Trim();
...
internal sealed class Collector
{
    ...
    public async Task<IngestRequest> BuildRequestAsync(string? scanMode, string? triggerType, AgentControlMonitor? controlMonitor, CancellationToken cancellationToken, Func<ScanProgressSnapshot, Task>? progressCallback, string? campusKey = null)
    {
        ...
        {
            CampusKey = (campusKey ?? string.Empty).Trim(),
...
internal sealed class CollectorOptions
{
    ...
    public string SourceName { get; set; } = "Pireon Network Collector";
