...
public class BackendAuthService
{
    ...
    {
        var existingSuperAdmin = await _context.AuthUsers
            .AnyAsync(user => user.Role == AppRoles.SuperAdmin && user.IsActive, cancellationToken);
        ...
        // Migración: si ya existe un admin sin superadmin, promover el más antiguo
        // para no exigir variables de entorno en instalaciones previas.
        var existingAdmin = await _context.AuthUsers
            .OrderBy(user => user.CreatedAtUtc)
            .FirstOrDefaultAsync(user => user.Role == AppRoles.Admin && user.IsActive, cancellationToken);

        if (existingAdmin is not null)
        {
            existingAdmin.Role = AppRoles.SuperAdmin;
        ...
        if (string.IsNullOrWhiteSpace(username) || string.IsNullOrWhiteSpace(password))
        {
            throw new InvalidOperationException(
                "No existe un administrador activo y faltan las variables de entorno " +
                "ADMIN_EMAIL / ADMIN_PASSWORD requeridas para crear el administrador inicial. " +
                "Configura ambas variables y reinicia la aplicacion.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"La contrasena del administrador inicial no cumple la politica de seguridad: {policyError}");
        }

        await EnsureUserAsync(username, password, AppRoles.SuperAdmin, cancellationToken);
        await NormalizeLegacyRolesAsync(cancellationToken);
    }
    ...
    public static string NormalizeRole(string? role)
    {
        ...
        {
            AppRoles.SuperAdmin => AppRoles.SuperAdmin,
    ...
    {
        var breakGlassUsernames = _configuration["AuthSettings:BreakGlassUsernames"] ?? string.Empty;
        var configuredAdmin = GetString("AuthSettings:AdminUsername", "ADMIN_EMAIL")
                              ?? _configuration["SeedUsers:Admin:Username"]
                              ?? string.Empty;
        var candidates = string.Join(',', new[] { breakGlassUsernames, configuredAdmin });
        var configuredUsers = candidates.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
