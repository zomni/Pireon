...
public class FrontendSyncService
{
    ...
    public async Task<FrontendSyncResult> SyncAsync(CancellationToken cancellationToken = default)
    {
        ...
        var catalogPath = Path.Combine(dataRoot, "pireon_buildings_catalog.json");

        if (!File.Exists(catalogPath))
        {
            throw new FileNotFoundException("No se encontró pireon_buildings_catalog.json en la ruta configurada.", catalogPath);
        }
        ...
        foreach (var building in buildings)
        {
            ...
            var syncedBuilding = new SyncedBuilding
            {
                ...
                Campus = CampusDefaults.Resolve(_configuration, null),
                ...
                IsActive = !(existingBuildingOverrides.GetValueOrDefault(building.Id)?.IsDeleted ?? false),
    ...
    {
        var mountedPath = _configuration["FrontendDataPath"];
        if (!string.IsNullOrWhiteSpace(mountedPath))
        {
            return Path.GetFullPath(mountedPath);
        }
        ...
        return Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "pireon", "src", "data"));
    }
