...
{
    private const string CachePrefix = "pireon:mfa:setup:";
        ...
        IConfiguration configuration)
    {
        _context = context;
        _cache = cache;
        _protector = dataProtectionProvider.CreateProtector("Pireon.API.MfaSecret.v1");
    ...
    public bool IsRequiredForRole(string role)
    {
        ...
        return normalizedRole is AppRoles.Admin or AppRoles.SuperAdmin
    ...
    public async Task<MfaSetupSession?> BeginEnrollmentAsync(AuthUser user, CancellationToken cancellationToken = default)
    {
        ...
        var issuer = GetString("MfaSettings:Issuer", "MFA_ISSUER") ?? "Pireon";
    ...
    public async Task<bool> ResetEnrollmentAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        var user = await _context.AuthUsers.SingleOrDefaultAsync(item => item.Id == userId, cancellationToken);
        if (user is null)
        {
