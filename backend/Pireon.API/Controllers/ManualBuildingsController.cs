...
public class ManualBuildingsController : ControllerBase
{
    ...
    public ManualBuildingsController(AppDbContext context, IConfiguration configuration, OrganizationAccessService access)
    {
        _context = context;
        ...
        var buildings = manualBuildings
            .Select(building =>
            {
                ...
                return new
                {
                    ...
                    IsDeleted = syncedBuilding?.IsActive == false
                };
            })
    ...
    public async Task<IActionResult> Delete(string externalId, CancellationToken cancellationToken)
    {
        ...
        if (!await _access.CanAccessCampusAsync(manualBuilding.Campus, cancellationToken))
            return Forbid();
