...
public class NetworkTelemetryOfficeController : ControllerBase
{
    ...
    public NetworkTelemetryOfficeController(NetworkTelemetryService service, OrganizationAccessService access)
    {
        _service = service;
        _access = access;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var dashboard = await _service.GetDashboardAsync(10, snapshotId, campusKeys, cancellationToken);
        if (!dashboard.HasData || dashboard.ActiveSnapshotId == Guid.Empty)
        {
    ...
    public async Task<IActionResult> SnapshotDetail(
        Guid snapshotId,
        [FromQuery] Guid? organizationId = null,
        CancellationToken cancellationToken = default)
    {
    ...
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [HttpGet("snapshots/{snapshotId:guid}/users")]
    ...
    {
        var campusKeys = await ResolveCampusKeysAsync(organizationId, cancellationToken);
        var result = await _service.GetSessionOverviewAsync(snapshotId, campusKeys, cancellationToken);
        return Ok(result);
    }
    ...
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [HttpGet("snapshots/{snapshotId:guid}/risks")]
    ...
    {
        var campusKeys = await ResolveCampusKeysAsync(organizationId, cancellationToken);
        var top = await _service.GetTopRiskObservationsAsync(snapshotId, take, campusKeys, cancellationToken);
        var subnet = await _service.GetSubnetRiskSummariesAsync(snapshotId, campusKeys, cancellationToken);
        var building = await _service.GetBuildingRiskSummariesAsync(snapshotId, campusKeys, cancellationToken);
    ...
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [HttpGet("snapshots/{snapshotId:guid}/devices/export")]
    ...
    {
        var campusKeys = await ResolveCampusKeysAsync(organizationId, cancellationToken);
        var exportData = await _service.GetSnapshotExportDataAsync(snapshotId, campusKeys, cancellationToken);
        if (exportData == null)
        {
            return NotFound(new { message = $"No se encontro el snapshot {snapshotId}." });
        }
