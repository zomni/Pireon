...
public class FrontendStaticBackupController : ControllerBase
{
    ...
    public FrontendStaticBackupController(AppDbContext context, IConfiguration configuration, OrganizationAccessService access)
    {
        _context = context;
    ...
    {
        var normalizedCampus = CampusDefaults.Resolve(_configuration, campus);

        if (string.IsNullOrWhiteSpace(normalizedCampus))
        {
            return BadRequest(new { message = "El campus es obligatorio." });
        }

        if (!await _access.CanAccessCampusAsync(normalizedCampus, cancellationToken))
        {
            return Forbid();
        }
