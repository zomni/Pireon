...
public class AdminController : Controller
{
    ...
    public record CreateUserRequest(string Username, string Password, string? Role, bool CanManageUsers, Guid? OrganizationId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        HashSet<string>? scopedBuildingIds = campusKeys is null
            ? null
            : await ResolveScopedBuildingExternalIdsAsync(campusKeys, cancellationToken);

        var databaseFileInfo = GetDatabaseFileInfo();
        var databaseBackups = GetDatabaseBackupFiles();

        var buildingsQuery = ApplyCampusScope(_context.SyncedBuildings.AsNoTracking(), campusKeys);
        var roomsQuery = _context.SyncedRooms.AsNoTracking();
        var inventoryQuery = ApplyInventoryScope(_context.ImportedInventoryItems.AsNoTracking(), scopedBuildingIds);

        if (scopedBuildingIds is not null)
        {
            roomsQuery = roomsQuery.Where(room => scopedBuildingIds.Contains(room.BuildingExternalId));
        }
        ...
        var model = new AdminDashboardViewModel
        {
            ...
            DatabaseFileName = databaseFileInfo?.Name ?? "pireon.db",
    ...
    {
        var currentUser = await _access.GetCurrentUserAsync(cancellationToken);
        ...
        query = _access.ScopeUsersQuery(query);
    ...
    public async Task<IActionResult> CreateUser([FromBody] CreateUserRequest request, CancellationToken cancellationToken)
    {
        ...
        if (role is AppRoles.SuperAdmin or AppRoles.Admin)
        {
            role = AppRoles.Editor;
        }

        Guid? organizationId = currentUser.OrganizationId;
        ...
        await _auditLogService.LogSecurityEventAsync(
            ...
            details: $"Rol: {user.Role}; Organizacion: {user.OrganizationId}",
            result: "success",
            severity: "info",
    ...
    public async Task<IActionResult> UpdateUser(Guid id, [FromBody] UpdateUserRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!_access.IsSuperAdmin && targetUser.OrganizationId != currentUser.OrganizationId)
        {
            return Forbid();
        }

        if (targetUser.Role == AppRoles.SuperAdmin && !_access.IsSuperAdmin)
        {
            return Forbid();
        }
        ...
        if (!string.IsNullOrWhiteSpace(request.Role))
        {
            ...
            if (!_access.IsSuperAdmin && newRole == AppRoles.SuperAdmin)
            {
        ...
        targetUser.CanManageUsers = request.CanManageUsers && targetUser.Role is AppRoles.Admin or AppRoles.SuperAdmin;
        ...
        CancellationToken cancellationToken)
    {
        ...
        ViewData["CampusKeys"] = campusKeys ?? await _context.CampusSites
            .AsNoTracking()
    ...
    public async Task<IActionResult> DownloadDatabase()
    {
        ...
        return DownloadDatabaseFile(databasePath, $"pireon-backup-{DateTime.UtcNow:yyyyMMdd-HHmmss}.db");
    }
    ...
    {
        var packageName = $"pireon-data-package-{DateTime.UtcNow:yyyyMMdd-HHmmss}.zip";
        var tempRoot = Path.Combine(Path.GetTempPath(), $"pireon-data-export-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            CopyFileIfExists(GetDatabaseFilePath(), Path.Combine(backendStaging, "pireon.db"));
            ...
            if (!string.IsNullOrWhiteSpace(frontendDataDirectory))
            {
                ...
                CopyFileIfExists(Path.Combine(frontendDataDirectory, "pireon_buildings_backend_backup.json"), Path.Combine(frontendStaging, "pireon_buildings_backend_backup.json"));
    ...
    public async Task<IActionResult> UploadProjectPackage(IFormFile? packageFile, CancellationToken cancellationToken)
    {
        ...
        var tempRoot = Path.Combine(Path.GetTempPath(), $"pireon-data-import-{Guid.NewGuid():N}");
    ...
    [Authorize(Roles = $"{AppRoles.Admin},{AppRoles.Editor},{AppRoles.Viewer},{AppRoles.Auditor},{AppRoles.SuperAdmin}")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var campusKeys = await ResolveScopeCampusKeysAsync(organizationId, cancellationToken);
        var scopedBuildingIds = campusKeys is null
            ? null
            : await ResolveScopedBuildingExternalIdsAsync(campusKeys, cancellationToken);

        var items = await ApplyInventoryScope(
                _context.ImportedInventoryItems.AsNoTracking(),
                scopedBuildingIds)
            .Select(item => new
            {
                item.SerialNumber,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var buildings = await ApplyCampusScope(
                _context.SyncedBuildings.AsNoTracking().Where(building => building.IsActive),
                campusKeys)
            .Select(building => new
            {
                building.ExternalId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var campuses = await ApplyCampusScope(
                _context.SyncedBuildings.AsNoTracking().Where(building => building.IsActive),
                campusKeys)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        buildingsQuery = ApplyCampusScope(buildingsQuery, campusKeys);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var inconsistencySnapshot = await AnalyzeInventoryInconsistenciesAsync(campusKeys);
        ...
        if (scopedBuildingIds is not null)
        {
            ...
            else
            {
                query = query.Where(item =>
                    item.AssignedBuildingExternalId != "" && scopedBuildingIds.Contains(item.AssignedBuildingExternalId) ||
                    item.MatchedBuildingExternalId != "" && scopedBuildingIds.Contains(item.MatchedBuildingExternalId));
            }
        }
        ...
        var model = new AdminInventoryListViewModel
        {
            ...
            Items = items,
            Buildings = await ApplyCampusScope(
                    _context.SyncedBuildings.AsNoTracking().Where(b => b.IsActive || b.ExternalId == buildingExternalId),
                    campusKeys)
            ...
            TotalItems = await ApplyInventoryScope(_context.ImportedInventoryItems.AsNoTracking(), scopedBuildingIds).CountAsync(cancellationToken),
            AssignedItems = await ApplyInventoryScope(_context.ImportedInventoryItems.AsNoTracking().Where(i => i.AssignedBuildingExternalId != ""), scopedBuildingIds).CountAsync(cancellationToken),
            PendingItems = await ApplyInventoryScope(_context.ImportedInventoryItems.AsNoTracking().Where(i => i.AssignedBuildingExternalId == ""), scopedBuildingIds).CountAsync(cancellationToken),
            SuggestedItems = await ApplyInventoryScope(_context.ImportedInventoryItems.AsNoTracking().Where(i => i.AssignedBuildingExternalId == "" && i.MatchedBuildingExternalId != ""), scopedBuildingIds).CountAsync(cancellationToken),
    ...
    private async Task<IReadOnlyList<string>?> ResolveScopeCampusKeysAsync(Guid? organizationId, CancellationToken cancellationToken)
    {
    ...
    private async Task<HashSet<string>> ResolveScopedBuildingExternalIdsAsync(IReadOnlyList<string> campusKeys, CancellationToken cancellationToken)
    {
        var externalIds = await _context.SyncedBuildings
            .AsNoTracking()
            .Where(building => building.IsActive &&
                               (campusKeys.Contains(building.Campus) || campusKeys.Contains(building.ManualCampus)))
    ...
    private IQueryable<SyncedBuilding> ApplyCampusScope(IQueryable<SyncedBuilding> query, IReadOnlyList<string>? campusKeys)
    {
        ...
        {
            return query.Where(building => false);
        }

        return query.Where(building => campusKeys.Contains(building.Campus) || campusKeys.Contains(building.ManualCampus));
    }

    private IQueryable<ImportedInventoryItem> ApplyInventoryScope(
        IQueryable<ImportedInventoryItem> query,
        IReadOnlyCollection<string>? scopedBuildingIds)
    {
        if (scopedBuildingIds is null)
        {
            return query;
        }
    ...
    private async Task SetOrganizationSelectorViewDataAsync(Guid? organizationId, CancellationToken cancellationToken)
    {
        ViewData["SelectableOrganizations"] = await _access.GetSelectableOrganizationsAsync(cancellationToken);
        ViewData["EffectiveOrganizationId"] = _access.EffectiveOrganizationId(organizationId);
    }

    private async Task<InventoryInconsistencySnapshot> AnalyzeInventoryInconsistenciesAsync(IReadOnlyList<string>? campusKeys = null)
    {
        var snapshot = new InventoryInconsistencySnapshot();
        var scopedBuildingIds = campusKeys is null
            ? null
            : await ResolveScopedBuildingExternalIdsAsync(campusKeys, CancellationToken.None);
    ...
    {
        var defaults = Pireon.API.Services.InventoryCategoriesConfig.GetCategoryNames(_configuration);
    ...
    {
        var defaults = Pireon.API.Services.InventoryCategoriesConfig.GetStatusNames(_configuration);
        var values = await _context.ImportedInventoryItems
            .AsNoTracking()
    ...
    {
        return Path.Combine(Path.GetTempPath(), "pireon-delivery-preview-cache");
    }
    ...
    private string? ResolveFrontendDataDirectory()
    {
        ...
        {
            var siblingFrontendData = Path.Combine(current.FullName, "..", "pireon", "src", "data");
            if (Directory.Exists(siblingFrontendData))
            {
                return Path.GetFullPath(siblingFrontendData);
            }

            var directFrontendData = Path.Combine(current.FullName, "pireon", "src", "data");
    ...
    private async Task RestoreProjectPackageAsync(string extractRoot, CancellationToken cancellationToken)
    {
        ...
        var dbSource = Path.Combine(backendPackageRoot, "pireon.db");
        if (!System.IO.File.Exists(dbSource))
        {
            throw new InvalidOperationException("El paquete no contiene pireon.db.");
        }
        ...
        if (!string.IsNullOrWhiteSpace(frontendDataDirectory) && Directory.Exists(frontendPackageRoot))
        {
            ...
            CopyFileIfExists(Path.Combine(frontendPackageRoot, "pireon_buildings_backend_backup.json"), Path.Combine(frontendDataDirectory, "pireon_buildings_backend_backup.json"));
