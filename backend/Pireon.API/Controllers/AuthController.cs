...
public class AuthController : Controller
{
    ...
    private const string RememberMeClaimType = "pireon:remember_me";
    private const string CanManageUsersClaimType = "pireon:can_manage_users";
    private const string OrganizationIdClaimType = "pireon:organization_id";
    private const string MfaModeClaimType = "pireon:mfa_mode";
    private const string MfaSetupKeyClaimType = "pireon:mfa_setup_key";
    private const string MfaUserIdClaimType = "pireon:mfa_user_id";
    private const string MfaReturnUrlClaimType = "pireon:mfa_return_url";
    ...
    public async Task<IActionResult> MfaSetup(MfaSetupViewModel model, CancellationToken cancellationToken)
    {
        ...
        if (!success)
        {
            ModelState.AddModelError(string.Empty, "El codigo MFA no es valido. Verifica que estes usando la entrada nueva de Pireon/admin y que la hora del telefono este sincronizada automaticamente.");
            var session = _mfaService.GetEnrollmentSession(pending.SetupKey);
            if (session is not null)
            {
                model.Username = pending.Username;
    ...
    public async Task<IActionResult> Session(CancellationToken cancellationToken)
    {
        if (User.Identity?.IsAuthenticated != true)
        {
            return Ok(new { isAuthenticated = false, username = "", role = "", isAdmin = false, isSuperAdmin = false, organizationId = "", organizationName = "", organizationColor = "", sites = Array.Empty<object>() });
        }
        ...
        var canManageUsers = string.Equals(
            ...
            "true",
            StringComparison.OrdinalIgnoreCase);
        var organizationIdValue = User.FindFirstValue(OrganizationIdClaimType);
        var isSuperAdmin = string.Equals(role, AppRoles.SuperAdmin, StringComparison.OrdinalIgnoreCase);

        Guid? organizationId = null;
        string organizationName = string.Empty;
        string organizationColor = string.Empty;
        if (Guid.TryParse(organizationIdValue, out var parsedOrganizationId))
        {
            organizationId = parsedOrganizationId;
            var orgData = await _context.Organizations
                .AsNoTracking()
                .Where(org => org.Id == parsedOrganizationId && org.IsActive)
                .Select(org => new { org.Name, org.Color })
                .FirstOrDefaultAsync(cancellationToken);
            organizationName = orgData?.Name ?? string.Empty;
            organizationColor = orgData?.Color ?? string.Empty;
        }

        var sitesQuery = _context.CampusSites
            .AsNoTracking()
            .Where(site => site.IsActive);

        if (organizationId.HasValue)
        {
            sitesQuery = sitesQuery.Where(site => site.OrganizationId == organizationId.Value);
        }
        ...
        var sites = await sitesQuery
            ...
            .Select(site => new
            {
                ...
                organizationColor = site.Organization.Color,
                school = site.School,
                floors = site.FloorsJson,
                defaultFloor = site.DefaultFloor,
                center = new[] { site.CenterLatitude, site.CenterLongitude },
                zoom = site.Zoom,
                minZoom = site.MinZoom,
                maxZoom = site.MaxZoom,
                bounds = new[]
                {
                    new[] { site.BoundsMinLatitude, site.BoundsMinLongitude },
                    new[] { site.BoundsMaxLatitude, site.BoundsMaxLongitude }
                }
            })
    ...
    private async Task SignInFinalUserAsync(AuthUser user, bool rememberMe, CancellationToken cancellationToken)
    {
        ...
        {
            claims.Add(new Claim(OrganizationIdClaimType, user.OrganizationId.Value.ToString()));
        }
