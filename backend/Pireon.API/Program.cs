...
builder.Services
    ...
    .AddCookie(options =>
    {
        ...
        options.Cookie.Name = "Pireon.Auth";
...
builder.Services.AddAuthentication()
    .AddCookie("MfaPending", options =>
    {
        ...
        options.Cookie.Name = "Pireon.MfaPending";
        options.Cookie.HttpOnly = true;
...
using (var scope = app.Services.CreateScope())
{
    ...
    await ExtendedSchemaInitializer.EnsureAsync(context, builder.Configuration);

    if (builder.Configuration.GetValue<bool>("DemoData:Enabled"))
    {
        await SeedData.InitializeAsync(context, builder.Configuration);
    }

    var authService = scope.ServiceProvider.GetRequiredService<BackendAuthService>();
    await authService.EnsureInitialAdminAsync();
}
...
app.Use(async (context, next) =>
{
    if (context.Request.Path.StartsWithSegments("/dashboard", out var dashboardRemaining))
    {
        if (dashboardRemaining.Equals("/network-telemetry", StringComparison.OrdinalIgnoreCase)
            || dashboardRemaining.StartsWithSegments("/network-telemetry", StringComparison.OrdinalIgnoreCase))
        {
            await next();
            return;
        }

        context.Request.Headers["X-Pireon-Public-Path"] = context.Request.Path.ToString();
